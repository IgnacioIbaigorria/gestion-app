      ...
      const productsWithTags = await Promise.all(sortedProducts.map(async (product) => {
        if (product.id) {
          ...
          if (product.tags && product.tags.length > 0) {
            ...
            const productTags = await tagService.getTagsForProduct(product.id);
            // Extract tag IDs
            const tagIds = productTags.map(tag => tag.id || '').filter(id => id !== '');
            // Update the product with its tags in the service cache
            productService.updateProductInCache(product.id, { 
              ...product, 
              tags: tagIds,
              tagObjects: productTags 
            });
            // Return the updated product
            return { ...product, tags: tagIds, tagObjects: productTags };
          }
        }
        return product;
      }));
