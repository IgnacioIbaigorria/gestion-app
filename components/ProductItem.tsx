...
export default function ProductItem({ product, onDelete, highlighted, category }: ProductItemProps) {
  ...
  useEffect(() => {
    ...
    if (!category && product.categoryId) {
      if (categoriesCache.length > 0) {
        const foundCategory = categoriesCache.find(cat => cat.id === product.categoryId);
        if (foundCategory) {
  ...
  }, [product.id, product.name, product.costPrice, product.sellingPrice, product.tags, product.tagObjects, category]); // Add specific dependencies
